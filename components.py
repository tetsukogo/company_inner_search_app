...
def display_app_title():
    ...
    """
    inject_css()
    st.markdown(f"<h2 class='page-title'>{ct.APP_NAME}</h2>", unsafe_allow_html=True)

    # 初期ガイダンス（緑・黄ボックス）
    with st.chat_message("assistant"):
        st.success(
            "こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。"
            "左側のサイドバーで利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。"
        )
        st.warning("具体的に入力したほうが的確な回答を得やすいです。", icon="⚠️")
